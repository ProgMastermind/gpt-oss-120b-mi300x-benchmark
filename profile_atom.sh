...
# The trace is written to /workspace/atom_profile and viewable in
# https://ui.perfetto.dev/.

# ATOM / ROCm environment knobs from start_server_atom_best.sh.
...
PROFILE_DIR=/workspace/atom_profile
mkdir -p "$PROFILE_DIR"

ATOM_LOG="atom_profile_server.log"
SERVER_PID=""

cleanup() {
    if [ -n "$SERVER_PID" ]; then
        echo "Stopping ATOM server (PID $SERVER_PID)..."
...
echo "Starting ATOM server with torch profiler..."
...
SERVER_PID=$!
echo "ATOM server PID: $SERVER_PID"
